...
function Invoice() {
    ...
    if (error || !order) {
        return <p className="text-center mt-5">ຂໍອະໄພ, ບໍ່ສາມາດໂຫຼດຂໍ້ມູນໃບເກັບເງິນສຳລັບເລກທີ່: {params.id}</p>;
    }
            ...
            <div className="order-invoice my-5">
                ...
                <div id="order_invoice" className="p-3 border border-secondary">
                    <header className="clearfix">
                        ...
                        <div id="project">
                            <div><span>ຊື່</span> {user?.name || 'N/A'}</div>
                            <div><span>ອີເມວ</span> {user?.email || 'N/A'}</div>
                            <div><span>ເບີໂທ</span> {shippingInfo?.phoneNo || 'N/A'}</div>
                            <div>
                                <span>ທີ່ຢູ່</span> 
                                {shippingInfo ? `${shippingInfo.address}, ${shippingInfo.city}, ${shippingInfo.zipCode}, ${shippingInfo.country}` : 'N/A'}
                            </div>
                            <div><span>ວັນທີ</span> {formatDate(createdAt)}</div>
                            {/* Payment Status */}
                            <div className={paymentInfo?.status === 'Paid' || paymentInfo?.status === 'succeeded' ? 'greenColor' : 'redColor'}>
                                <span>ສະຖານະ</span> {paymentInfo?.status === 'Paid' || paymentInfo?.status === 'succeeded' ? 'ຊຳລະແລ້ວ' : orderStatus}
                            </div>
                        </div>
                    </header>
                    ...
                    <main>
                        ...
                        <div id="notices">
                            ...
                            <div className="notice">
                                ຈະມີການເອີ້ນເກັບເງິນຄ່າທຳນຽມ 1.5% ຈາກຍອດ ຄົງເຫຼືອທີ່ຍັງບໍ່ໄດ້ຊຳລະຫຼັງຈາກ 30 ວັນ.
                            </div>
                        </div>
                    </main>
                    
                    <footer>
                        ໃບເກັບເງິນຖືກສ້າງຂຶ້ນດ້ວຍລະບົບຄອມພິວເຕີ ແລະ ສາມາດນໍາໃຊ້ໄດ້ໂດຍບໍ່ມີລາຍເຊັນ.
                    </footer>
                </div>
                {/* INVOICE CONTENT - END */}
            </div>
